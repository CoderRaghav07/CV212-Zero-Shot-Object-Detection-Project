...
def detect_open_vocab(image_path, class_prompts, device="cpu",
    ...
    print(f"[INFO] After NMS: {len(detections)}")

    return np_img, detections, run_ocr(pil_img, min_confidence=60)

def demo(args):
    class_prompts = ["Bins with recycle symbol", "crumbled plastic wrappers", "Plastic bottle"]
    ...
    color_map = {
        "Bins with recycle symbol": "green",
        "Plastic bottle": "red",
        "crumbled plastic wrappers": "orange" ,
    ...
    print(f"[RESULT] Saved annotated detections + OCR text to {out_path}")
